$(document).ready(function () {
  ...
  for(i = 0; i < dataShapes.length; i++){    // loops over dataShapes array
    var currentShape = dataShapes[i];        // dataShapes[i] copies the value of dataShapes array
     
    if(currentShape.color === "red"){            // color of the currentShape
       currentShape.goodBehavior = "bounce";     // if it's red, then bounce
     } else if (currentShape.color === "blue"){
        currentShape.goodBehavior = "blink";     // if it's blue, then blink
     } else {
        currentShape.goodBehavior = "spin";      // if it's any other color, then spin
     }
  }
  

  // TODO 3-a: add a function that handles the static display type
  function handleStatic(data){                 // data is a parameter
    setBackgroundWithObject(data);             // passes data into it as an argument
    animationDetails.displayType = 1;
  }

  // TODO 4-a: add a function that handles the good display type
  function handleGood(color, shape, repeat){
    setBackgroundWithSimple(color, shape, repeat);       // calls setBackgroundWithSimple and passes all the arguments
  ...
  function staticDisplay() {
    ...
    handleStatic(dataShapes[currentIndex]);        // dataShapes is array, currentIndex is a var, passes through handleStatic
  }
